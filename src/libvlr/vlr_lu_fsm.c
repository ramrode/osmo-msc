...
static void vlr_loc_upd_node1(struct osmo_fsm_inst *fi)
{
	...
	if (try_auth(lfp)) {
		...
		osmo_fsm_inst_state_chg(fi, VLR_ULA_S_WAIT_AUTH,
					LU_TIMEOUT_LONG, 0);
		/* First only create and assign vsub->auth_fsm, because an auth_fsm_start() might run into a failure
		 * and the fsm will be free'ed.
		 */
...
static bool location_served(struct vlr_instance *vlr,
			    struct lu_fsm_priv *lfp)
{
	if (vlr_is_cs(vlr))
		return vlr->ops.location_area_served(lfp->vsub, &lfp->old_lai);
	else
		return vlr->ops.routing_area_served(lfp->vsub, &lfp->old_rai);
}
...
static void lu_fsm_wait_pvlr_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	if (vlr_is_cs(vlr))
		vlr->ops.tx_pvlr_request_lai(lfp->msc_conn_ref, &lfp->old_lai);
	else
		vlr->ops.tx_pvlr_request_rai(lfp->msc_conn_ref, &lfp->old_rai);
}
